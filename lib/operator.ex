defmodule Challenge.Operator do
  ...
  #   # check that a bet exist for this win
  #   # check the win is not regsitered in our store as of yet
  #   # finally get user and update amount
  #   # on failure send messages to self to keep retrying
  # end

  @spec get_user(name :: String.t()) ::  {:ok, name :: String.t(), amount :: number(), currency :: String.t()}
  | :error
  | :does_not_exit
  ...
  @spec update_transactions_table(trans_id :: binary(), name :: String.t(), status :: boolean()) :: :ok | :error
  def update_transactions_table(trans_id, name, status) do
    Server.add_transaction_uuid(trans_id, name, status)
  end
  @spec update_user(name :: String.t(), amount :: String.t()) :: :failed_to_update_user | :error | :ok
