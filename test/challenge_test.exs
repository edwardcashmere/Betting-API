defmodule ChallengeTest do
  ...
  test "create users works with an list of users", %{
    server_pid: pid,currency: currency, amount: amount
  } do
  ...
  test "bet works as expected", %{
    server_pid: pid, currency: currency
  } do
    users = ["john12345"]
    Challenge.create_users(pid, users)
    Process.sleep(1)
    ...
    currency: "USD",
    balance: 0} = Challenge.bet(pid, bet)

    assert {:ok, "john12345", 0 , ^currency} = Server.get_user("john12345")
  end


  test "a bet can be made with valid data with -ve amount value", %{server_pid: pid, currency: currency} do
    users = ["john1"]
    Challenge.create_users(pid, users)
    Process.sleep(1)
    ...
    assert {:ok, "john1", 0 , ^currency} = Server.get_user("john1")
  end

  test "a bet that was already processed is not processed again", %{server_pid: pid, currency: currency} do
    users = ["john12"]
    Challenge.create_users(pid, users)
    Process.sleep(1)
  ...
  test "a bet cannot be made when the user does not exist", %{server_pid: pid} do
    users = ["john12"]
    Challenge.create_users(pid, users)
    Process.sleep(1)
  ...
  test "a win can be made with valid data", %{server_pid: pid, currency: currency} do
    ...
    request_uuid: "583c985f-fee6-4c0e-bbf5-308aad6265af",
    currency: ^currency,
    balance: 200_000} = Challenge.win(pid, win)

    assert {:ok, "john12345", 200_000 , ^currency} = Server.get_user("john12345")
  end

  test "a win can be made with valid data with -ve amount value", %{server_pid: pid, currency: currency} do
    users = ["john1234"]
    Challenge.create_users(pid, users)
    Process.sleep(1)
    ...
    assert {:ok, "john1234", 200_000 , ^currency} = Server.get_user("john1234")

  end

  test "a win that was already processed is not processed again", %{server_pid: pid, currency: currency} do
    users = ["john12"]
    Challenge.create_users(pid, users)
    Process.sleep(1)
    ...
    assert {:ok, "john12", 200_000 , ^currency} = Server.get_user("john12")
  ...
  test "a win cannot be made when the user does not exist", %{server_pid: pid, currency: currency} do
    ...
    request_uuid: "583c985f-fee6-4c0e-bbf5-308aad6265af",
    currency: ^currency,
    balance: 0} = Challenge.win(pid, win)
...
defp test_bet_data(data_type) do
  case data_type do
    ...
    :negative_amount_data ->
      %{
        user: "john1234",
        transaction_uuid: "16d2dcfe-b89e-11e7-854a-58dfhk324",
        supplier_transaction_id: "41ecc3ad-b181-4235-bf9d-acf0a7ad9730",
        token: "55b7518e-b89e-11e7-81be-58404eea6d16",
        supplier_user: "cg_45141",
        round_closed: true,
    ...
    :duplicate_trans_id_data ->
      %{
        user: "john12",
        transaction_uuid: "111111111111111111111111",
        supplier_transaction_id: "41ecc3ad-b181-4235-bf9d-acf0a7ad9730",
        token: "55b7518e-b89e-11e7-81be-58404eea6d16",
        supplier_user: "cg_45141",
        round_closed: true,
